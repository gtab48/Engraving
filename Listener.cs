...
namespace Engraving
{
    internal class Listener
    {
        ...
        {
            //定义url及端口号，通常设置为配置文件
            listener.Prefixes.Add("http://+:8080/Home/");
            //启动监听器
            listener.Start();
        ...
        private static object HttpData(string data)
        {
            ...
            object[] parameters = jobject.RootData().ToArray();
            if (Equals(jobject.name, "Engraving") || Equals(jobject.name, "EngravingAll") || Equals(jobject.name, "MarkEntity")) 
            {
                parameters = jobject.Data().ToArray();
            }
            parameters = parameters.Where(s => s != null).ToArray();
            if (method != null)
            {
                try
                {
